...
//			}else{
	...
	addGiftIdea: async (ev)=>{
		...
		let online = document.getElementById('online'),
			...
			num = 0;
		if(!validInput){
			let message = "Please Enter Gift Name";
		...
		if(num == -1){
			...
			let message = "Price must be a number \n \n Please enter url in the format 'http://www.example.com'";
			app.displayOverlay(message, 3000);
        ...
        if((validInput && validPriceInt && validPrice && url == "" && validPriceInt)||(validInput && validPrice && validPriceInt && num != -1 && validPriceInt)){
